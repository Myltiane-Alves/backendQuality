...
export const getProdutosPedidos = async (
    ...
    try {
        ...
        let query = `
            ...
            FROM 
                ...
                INNER JOIN "${databaseSchema}".DETALHEPEDIDO DP ON tbdp.IDDETALHEPEDIDO = DP.IDDETALHEPEDIDO
                INNER JOIN "${databaseSchema}".RESUMOPEDIDO RP ON DP.IDRESUMOPEDIDO = RP.IDRESUMOPEDIDO
        ...
        if (idMarca) {
            query += ' AND RP.IDSUBGRUPOEMPRESARIAL = ?';
            params.push(idMarca);
        }

        if (idFornecedor) {
            query += ' AND RP.IDFORNECEDOR = ?';
        ...
        console.error('Error executar a consulta produtos de pedidos ', error);
        throw error;
    }
};
