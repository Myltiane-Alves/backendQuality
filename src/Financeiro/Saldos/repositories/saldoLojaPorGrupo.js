  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        tbe.NUCNPJ, 
        tbe.NOFANTASIA, 
        (SELECT (sum (cast(VRFISICODINHEIRO as decimal)) - sum(CASE WHEN VRAJUSTDINHEIRO >0 THEN VRAJUSTDINHEIRO ELSE VRRECDINHEIRO END)) 
        FROM "${databaseSchema}".MOVIMENTOCAIXA 
        WHERE (DTABERTURA BETWEEN '2020-12-11 00:00:00' AND '${dataPesquisa} 23:59:59') 
        AND IDEMPRESA = tbe.IDEMPRESA) AS TOTALQUEBRA, 
    ...
    console.error('Erro ao executar a consulta de saldo por loja:', error);
    throw error;
  }
};
