...
export const getLinhasDaVendaPagamento = async (idVenda) => {
    try {
        const query = `
            SELECT 
    ...
    try {
        ...
        let query = `
            SELECT 
                ...
                tbv.VRRECCHEQUE, 
                tbv.VRRECVOUCHER, 
                (SELECT IFNULL (SUM(tbvp.VALORRECEBIDO),0) FROM "${databaseSchema}".VENDAPAGAMENTO tbvp WHERE tbvp.IDVENDA = tbv.IDVENDA AND (tbvp.STCANCELADO = 'False' OR tbvp.STCANCELADO IS NULL) AND tbvp.NOTEF = 'POS' AND (tbvp.DSTIPOPAGAMENTO!='PIX')) AS TOTALVENDIDOPOS, 
                (SELECT IFNULL (SUM(tbvp.VALORRECEBIDO),0) FROM "${databaseSchema}".VENDAPAGAMENTO tbvp WHERE tbvp.IDVENDA = tbv.IDVENDA AND (tbvp.STCANCELADO = 'False' OR tbvp.STCANCELADO IS NULL) AND tbvp.NOTEF = 'PIX' AND (tbvp.DSTIPOPAGAMENTO ='PIX')) AS TOTALVENDIDOPIX, 
                (SELECT IFNULL (SUM(tbvp.VALORRECEBIDO),0) FROM "${databaseSchema}".VENDAPAGAMENTO tbvp WHERE tbvp.IDVENDA = tbv.IDVENDA AND (tbvp.STCANCELADO = 'False' OR tbvp.STCANCELADO IS NULL) AND tbvp.NOTEF = 'POS' AND (tbvp.DSTIPOPAGAMENTO ='MoovPay')) AS TOTALVENDIDOMOOVPAY 
            FROM 
      ...
      const data = await Promise.all(rows.map(async (registro) => {
        try {
            const vendaPagamento = await getLinhasDaVendaPagamento(registro.IDVENDA);
            return {
                "venda": {
            ...
            console.error('Erro ao executar a consulta de pagamentos venda:', error);
            throw error;
        }
        }));
        return {
        ...
        console.error('Erro ao executar a consulta de recebimentos:', error);
        throw error;
    }
}