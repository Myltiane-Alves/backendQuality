...
export const createMigracaoProdutos = async (codPedido) => {
    try {
        let query = `
            SELECT 
            ...
            WHERE 1=? AND (T1.STMIGRADOSAP != 'True') AND T1."IDRESUMOPEDIDO" = ${parseInt(codPedido)}
        `;
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (i === 0) {
                sessionId = await loginServiceLayer();
            }

            if (det.STREPOSICAO === 'True') {
                const retPat = await patchSl(`https://your-service-layer-url/b1s/v1/Items('${det.IDPRODCADASTRO}')`, Str_Json, sessionId);
                if (retPat === 'True') {
                    const resultMigracaoRep = await conn.exec(`SELECT "ItemCode" FROM "SBO_GTO_PRD".OITM WHERE 1=? AND "ItemCode" = '${det.IDPRODCADASTRO}'`, [1]);
                    if (resultMigracaoRep.length > 0) {
                        await atualizaMigracaoProdutos(det.IDPRODCADASTRO);
                    }
                } else {
                    return retPat;
                }
            } else {
                const retPost = await postSl('https://your-service-layer-url/b1s/v1/Items', Str_Json, sessionId);
                if (retPost === 'True') {
                    const resultMigracao = await conn.exec(`SELECT "ItemCode" FROM "SBO_GTO_PRD".OITM WHERE 1=? AND "ItemCode" = '${det.IDPRODCADASTRO}'`, [1]);
                    if (resultMigracao.length > 0) {
                        await updateMigracaoProdutos(det.IDPRODCADASTRO);
                    }
                } else {
                    return retPost;
                }
...
export const updateMigracaoProdutos = async (dados) => {
    try {
        const query = `
        ...
        return {
            status: 'success',
            message: 'Atualização Migração SAP realizada com sucesso!',
        };
    } catch (error) {
        throw new Error(`Erro ao atualização Migração SAP: ${error.message}`);
    }
};
